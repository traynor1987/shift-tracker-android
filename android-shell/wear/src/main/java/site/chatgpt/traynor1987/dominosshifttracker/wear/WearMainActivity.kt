...
class WearMainActivity : Activity(), DataClient.OnDataChangedListener {
    ...
    private fun render(){val s=WearState.read(this);actions.removeAllViews();if(s==null||s.disconnected){dial.accent=Color.rgb(224,163,56);dial.progress=.15f;state.text="PHONE DISCONNECTED";timer.stop();timer.text="LAST STATE";detail.text="Open phone to reconnect";return};if(!s.active){dial.accent=Color.rgb(8,117,209);dial.progress=.22f;state.text="CLOCKED OUT";timer.stop();timer.text="OFF";detail.text="Open phone to start a shift";return}
        ...
        listOf("delivered" to "DELIVERED","back_at_store" to "BACK","end_break" to "END","complete_task" to "DONE","single" to "SINGLE","double" to "DOUBLE","break" to "BREAK").filter{it.first in s.actions}.forEach{(a,l)->actions.addView(button(l,a,if(a=="single"||a=="delivered"&&s.activity=="delivery_single")Color.rgb(239,29,69)else if(a=="double"||a=="delivered")Color.rgb(8,117,209)else Color.rgb(28,157,130)))};actions.requestLayout()}
    private fun button(l:String,a:String,c:Int)=Button(this).apply{text=l;textSize=9f;setTextColor(Color.WHITE);background=GradientDrawable().apply{cornerRadius=25f;setColor(c);setStroke(2,Color.argb(190,255,255,255))};elevation=5f;setOnClickListener{isEnabled=false;state.text="$l…";WearTransport.sendAction(this,a);resync()}}
