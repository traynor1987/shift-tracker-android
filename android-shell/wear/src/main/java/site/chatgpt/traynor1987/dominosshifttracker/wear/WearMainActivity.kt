...
class WearMainActivity : Activity(), DataClient.OnDataChangedListener {
    ...
    private fun button(l:String,a:String,c:Int)=Button(this).apply{text=l;textSize=10f;gravity=Gravity.CENTER;includeFontPadding=false;isAllCaps=false;setPadding(2,0,2,0);setTextColor(Color.WHITE);background=GradientDrawable().apply{cornerRadius=24f;setColor(c);setStroke(2,Color.argb(210,255,255,255))};elevation=5f;setOnClickListener{isEnabled=false;state.text="$l…";WearTransport.sendAction(this@WearMainActivity,a);resync()}}
