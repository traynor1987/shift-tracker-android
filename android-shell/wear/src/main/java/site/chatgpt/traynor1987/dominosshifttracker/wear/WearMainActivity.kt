...
class WearMainActivity : Activity(), DataClient.OnDataChangedListener {
    ...
    private fun render(){val s=WearState.read(this);actions.removeAllViews();if(s==null||s.disconnected){dial.accent=Color.rgb(224,163,56);dial.progress=.15f;state.text="PHONE DISCONNECTED";timer.stop();timer.text="LAST STATE";detail.text="Open phone to reconnect";return};if(!s.active){dial.accent=Color.rgb(8,117,209);dial.progress=.22f;state.text="CLOCKED OUT";timer.stop();timer.text="OFF";detail.text="Open phone to start a shift";actions.addView(openPhoneButton());actions.requestLayout();return}
