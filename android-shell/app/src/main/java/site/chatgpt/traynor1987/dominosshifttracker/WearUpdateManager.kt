...
class WearUpdateManager(private val context: Context, private val executor: ExecutorService, private val emit: (JSONObject) -> Unit) {
    ...
    private fun discover():Candidate { val a=JSONArray(text(RELEASES,2_000_000)); val r=(0 until a.length()).map{a.getJSONObject(it)}.firstOrNull{!it.optBoolean("draft")&&!it.optBoolean("prerelease")}?:error("No stable release"); val assets=r.getJSONArray("assets"); fun asset(n:String)=(0 until assets.length()).map{assets.getJSONObject(it)}.firstOrNull{it.optString("name")==n}?:error("asset missing"); val root=JSONObject(text(asset(METADATA).getString("browser_download_url"),131072)); require(root.optString("channel")=="stable"); val w=root.optJSONObject("wear")?:error("Wear update unavailable"); val n=w.optString("versionName");val c=w.optLong("versionCode",0);val f=w.optString("asset");val h=w.optString("sha256").lowercase();require(n.matches(Regex("\\d+\\.\\d+\\.\\d+"))&&c>0&&f=="Shift-Tracker-Wear-$n.apk"&&h.matches(Regex("[a-f0-9]{64}")));return Candidate(n,c,f,h,asset(f).getString("browser_download_url")) }
