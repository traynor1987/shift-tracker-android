...
class WearUpdateManager(private val context: Context, private val executor: ExecutorService, private val emit: (JSONObject) -> Unit) {
    ...
    fun send() { val c=candidate; val installed=version(context); if(c==null||installed==null||c.code<=installed.second){emit(state("no_verified_update"));return}; emit(state("downloading")); executor.execute { var apk:File?=null; runCatching { val id=node()?:error("Watch disconnected"); apk=download(c); verify(apk!!,c.code); emit(state("verified")); emit(state("sending_to_watch").put("latestVersion",c.name)); transfer(id,apk!!,c); state("sent_to_watch").put("latestVersion",c.name).put("message","Ready to install on watch") }.onSuccess(emit).onFailure{emit(state("failed").put("message","Wear update was not sent. Existing apps are unchanged."))}; apk?.delete() } }
