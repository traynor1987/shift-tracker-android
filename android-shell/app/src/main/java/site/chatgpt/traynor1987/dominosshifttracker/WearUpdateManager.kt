...
class WearUpdateManager(private val context: Context, private val executor: ExecutorService, private val emit: (JSONObject) -> Unit) {
    companion object {
        ...
        fun recordVersion(c: Context, n: String, v: Long) { if (n.matches(Regex("\\d+\\.\\d+\\.\\d+")) && v > 0) c.getSharedPreferences(PREFS, 0).edit().putString("name", n).putLong("code", v).putLong("seen_at", System.currentTimeMillis()).apply() }
    ...
    fun check(manual:Boolean) {
        ...
        executor.execute {
            runCatching {
                val release = discover()
                candidate = release
                val nodeId = node()
                val watch = nodeId?.let(::requestFreshVersion)
                state(when {
                    nodeId == null -> "watch_not_connected"
                    watch == null -> "watch_version_unknown"
                    release.code > watch.second -> "available"
                    else -> "up_to_date"
                }).put("latestVersion", release.name)
                    .put("latestVersionCode", release.code)
                    .put("updateAvailable", watch?.let { release.code > it.second } ?: false)
            }.onSuccess(emit).onFailure {
                emit(state("unavailable").put("message", "Could not check Wear OS updates. Nothing changed."))
            }
        }
    }
    fun send() { val c=candidate; val installed=version(context); if(c==null||installed==null||c.code<=installed.second){emit(state("no_verified_update"));return}; emit(state("downloading")); executor.execute { var apk:File?=null; val report={done:Long,total:Long->emit(state("sending_to_watch").put("latestVersion",c.name).put("transferBytes",done).put("transferTotalBytes",total))}; runCatching { val id=node()?:error("Watch disconnected"); apk=download(c); verify(apk!!,c.code); emit(state("verified")); report(0,apk!!.length()); runCatching{transfer(id,apk!!,c,report)}.getOrElse{first->emit(state("sending_to_watch").put("latestVersion",c.name).put("message","Retrying the secure watch transfer…"));transfer(node()?:throw first,apk!!,c,report)}; state("sent_to_watch").put("latestVersion",c.name).put("message","Ready to install on watch") }.onSuccess(emit).onFailure{failure->emit(state("failed").put("message",("Wear update was not sent: "+(failure.message?:"connection interrupted")+". Existing apps are unchanged."))) }; apk?.delete() } }
    ...
    private fun verify(f:File, code:Long){
        val flags=PackageManager.GET_SIGNING_CERTIFICATES.toLong()
        val archive=if(Build.VERSION.SDK_INT>=33) context.packageManager.getPackageArchiveInfo(f.path,PackageManager.PackageInfoFlags.of(flags))
            else @Suppress("DEPRECATION") context.packageManager.getPackageArchiveInfo(f.path,PackageManager.GET_SIGNING_CERTIFICATES)
        require(archive!=null&&archive.packageName==context.packageName&&archive.longVersionCode==code)
        if(Build.VERSION.SDK_INT>=28){
            val own=if(Build.VERSION.SDK_INT>=33) context.packageManager.getPackageInfo(context.packageName,PackageManager.PackageInfoFlags.of(flags))
                else @Suppress("DEPRECATION") context.packageManager.getPackageInfo(context.packageName,PackageManager.GET_SIGNING_CERTIFICATES)
            val a=archive.signingInfo?.apkContentsSigners.orEmpty().map{hex(MessageDigest.getInstance("SHA-256").digest(it.toByteArray()))}.toSet()
            val b=own.signingInfo?.apkContentsSigners.orEmpty().map{hex(MessageDigest.getInstance("SHA-256").digest(it.toByteArray()))}.toSet()
            require(a.isNotEmpty()&&a==b)
        }
    }
    private fun transfer(id:String,f:File,c:Candidate,progress:(Long,Long)->Unit){val token="wear-${System.currentTimeMillis()}";val prep=JSONObject().put("token",token).put("versionName",c.name).put("versionCode",c.code).put("apkFile",c.file).put("sha256",c.sha).put("size",f.length()).toString().toByteArray();Tasks.await(Wearable.getMessageClient(context).sendMessage(id,PREPARE,prep),20,TimeUnit.SECONDS);runCatching {
        androidx.wear.remote.interactions.RemoteActivityHelper(context, java.util.concurrent.Executor { it.run() })
            .startRemoteActivity(android.content.Intent(android.content.Intent.ACTION_VIEW)
                .setData(android.net.Uri.parse("shifttracker://wear-update"))
                .addCategory(android.content.Intent.CATEGORY_BROWSABLE), id)
    };val ch=Tasks.await(Wearable.getChannelClient(context).openChannel(id,CHANNEL),20,TimeUnit.SECONDS);try{DataOutputStream(Tasks.await(Wearable.getChannelClient(context).getOutputStream(ch),30,TimeUnit.SECONDS)).use{o->val h=JSONObject().put("token",token).toString().toByteArray();o.writeInt(h.size);o.write(h);var sent=0L;var reported=-1;val b=ByteArray(32768);FileInputStream(f).use{i->while(true){val n=i.read(b);if(n<0)break;o.write(b,0,n);sent+=n;val pct=(sent*100/f.length()).toInt();if(pct!=reported){reported=pct;progress(sent,f.length())}}};o.flush()}}finally{Wearable.getChannelClient(context).close(ch)} }
    private fun node()=runCatching{Tasks.await(Wearable.getNodeClient(context).connectedNodes,15,TimeUnit.SECONDS).maxByOrNull{if(it.isNearby)1 else 0}?.id}.getOrNull()
    private fun requestFreshVersion(nodeId:String):Pair<String,Long>? {
        val requestedAt=System.currentTimeMillis()
        Tasks.await(Wearable.getMessageClient(context).sendMessage(nodeId,VERSION,byteArrayOf()),10,TimeUnit.SECONDS)
        val deadline=System.currentTimeMillis()+4_000L
        while(System.currentTimeMillis()<deadline){
            val prefs=context.getSharedPreferences(PREFS,0)
            if(prefs.getLong("seen_at",0L)>=requestedAt) return version(context)
            Thread.sleep(100L)
        }
        return null
    }
